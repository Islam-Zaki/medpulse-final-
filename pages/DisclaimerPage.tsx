...
import { DISCLAIMER_PAGE_CONTENT } from '../constants';

interface DisclaimerPageProps {
  navigate: NavigateFunction;
}

const DisclaimerSection: React.FC<{ icon: string; title: LocalizedString; children: React.ReactNode }> = ({ icon, title, children }) => {
...
const DisclaimerPage: React.FC<DisclaimerPageProps> = ({ navigate }) => {
  const { t } = useLocalization();
  const c = DISCLAIMER_PAGE_CONTENT;
    ...
    <div className="bg-white font-arabic">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 max-w-4xl">
        {c.sections.map((section, index) => (
            <DisclaimerSection key={index} icon={section.icon} title={section.title}>
                {section.content.map((p, i) => <p key={i}>{t(p)}</p>)}
            </DisclaimerSection>
        ))}
      </div>
    </div>
